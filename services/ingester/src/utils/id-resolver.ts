import { type AtprotoData, IdResolver, MemoryCache } from '@atproto/identity'
...
export function createBidirectionalResolver(resolver: IdResolver): BidirectionalResolver {
  return {
    ...
    async resolveDidToDidDoc(did: string): Promise<AtprotoData> {
      return resolver.did.resolveAtprotoData(did)
    },
