...
const logger = pino({ name: 'generator-handler' })

export async function handleGeneratorEvent(evt: NormalizedEvent, db: Database): Promise<void> {
...
async function handleDelete(evt: NormalizedEvent, db: Database): Promise<void> {
  try {
    ...
    if (result.deletedCount > 0) {
      logger.info({ uri: evt.uri }, 'Successfully removed generator from database')
    } else {
      logger.warn({ uri: evt.uri }, 'Generator not found in database for deletion')
    }
  } catch (error) {
    logger.error({ error, uri: evt.uri }, 'Failed to delete generator from database')
  }
}