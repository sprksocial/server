...
export async function transformPostToPostView(
  ...
  if (post.embed?.$type === "so.sprk.embed.images") {
    embed = {
      ...
      images: post.embed.images.map((img: SoSprkEmbedImages.Image) => ({
        thumb:
          `https://media.sprk.so/img/medium/${post.authorDid}/${img.image.ref.toString()}/webp`,
        fullsize:
          `https://media.sprk.so/img/full/${post.authorDid}/${img.image.ref.toString()}/webp`,
        alt: img.alt,
