  ...
  server.com.atproto.admin.getAccountInfos({
    ...
    handler: async ({ params, auth }) => {
      ...
      const infos = await Promise.all(mapDefined(dids, async (did) => {
        ...
        if (!info) return
        const profileRecord = await ctx.db.models.Profile.findOne({ authorDid: did })
        const profileObj = profileRecord ? { ...profileRecord.toJSON(), _id: undefined, __v: undefined } : undefined
      ...
      return {
        ...
        body: {
          infos: infos.filter((info): info is NonNullable<typeof info> => info != null),
        }
      }
    },
  })
