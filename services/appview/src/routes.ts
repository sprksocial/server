...
import * as Music from "#/lexicon/types/app/faisca/music";
...
export const createRouter = (ctx: AppContext) => {
  ...
  router.post(
    ...
    handler(async (req, res) => {
      ...
      const rkey = TID.nextStr()
      const record = {
        $type: 'app.faisca.music',
        songArtist: req.body?.songArtist,
        createdAt: new Date().toISOString(),
        songName: req.body?.songName,
        albumCover: req.body?.albumCover,
      } as Music.Record
      if (!Music.validateRecord(record).success) {
        return res
          .status(400)
          .type('html')
          .send('<h1>Error: Invalid music record</h1>')
      }
