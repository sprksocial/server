...
const id = "so.sprk.richtext.facet";

/** Annotation of a sub-string within rich text. */
export interface Main {
  $type?: "so.sprk.richtext.facet";
  index: ByteSlice;
  features:
    ($Typed<Mention> | $Typed<Link> | $Typed<Tag> | { $type: string })[];
}
...
export interface Mention {
  $type?: "so.sprk.richtext.facet#mention";
  did: string;
}
...
export interface Link {
  $type?: "so.sprk.richtext.facet#link";
  uri: string;
}
...
export interface Tag {
  $type?: "so.sprk.richtext.facet#tag";
  tag: string;
}
...
export interface ByteSlice {
  $type?: "so.sprk.richtext.facet#byteSlice";
  byteStart: number;
  byteEnd: number;
}
